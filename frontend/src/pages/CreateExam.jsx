...
export const CreateExam = () => {
    ...
    const handleCreateExam = async (e) => {
        ...
        try {
            ...
            setCreatedExamId(response.data.id || Math.random().toString(36).substr(2, 9)); // fallback ID if needed
            setStep(2);
    ...
    const handleSaveQuestions = async () => {
        ...
        setError(null);
        try {
            await api.post(`/api/instructor/exam/${createdExamId}/questions`, { questions });
            navigate('/dashboard');
        } catch (err) {
    ...
    const renderMCQForm = (q, i) => (
        <div className="space-y-4">
            <div className="space-y-2">
                <label className="text-sm font-medium text-slate-300">Question Title / Text</label>
                <textarea
                    value={q.question}
    ...
    const renderWrittenForm = (q, i) => (
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                <label className="text-sm font-medium text-slate-300">Marks Allocated</label>
                <input
                    type="number"
                    value={q.marks}
                    onChange={(e) => updateQuestion(i, 'marks', e.target.value)}
        ...
        <div className="space-y-6">
            <div className="space-y-2">
                ...
                <textarea
                    ...
                    onChange={(e) => updateQuestion(i, 'problemStatement', e.target.value)}
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    ...
                    <button type="button" onClick={() => {
                        const newCases = [...q.sampleTestCases, { input: '', output: '', explanation: '' }];
                        updateQuestion(i, 'sampleTestCases', newCases);
                    }} className="text-xs text-blue-400 hover:text-blue-300">+ Add Sample</button>
                </div>
                {q.sampleTestCases.map((tc, tcIdx) => (
                    <div key={tcIdx} className="grid grid-cols-1 gap-2 p-3 bg-slate-900/80 rounded-lg border border-white/5">
                        <textarea placeholder="Input" value={tc.input} onChange={(e) => {
                            const newCases = [...q.sampleTestCases]; newCases[tcIdx].input = e.target.value; updateQuestion(i, 'sampleTestCases', newCases);
                        }} className="w-full bg-slate-950 p-2 text-sm rounded border border-white/5" />
                        <textarea placeholder="Output" value={tc.output} onChange={(e) => {
                            const newCases = [...q.sampleTestCases]; newCases[tcIdx].output = e.target.value; updateQuestion(i, 'sampleTestCases', newCases);
                        }} className="w-full bg-slate-950 p-2 text-sm rounded border border-white/5" />
                        <textarea placeholder="Explanation" value={tc.explanation} onChange={(e) => {
                            const newCases = [...q.sampleTestCases]; newCases[tcIdx].explanation = e.target.value; updateQuestion(i, 'sampleTestCases', newCases);
                        }} className="w-full bg-slate-950 p-2 text-sm rounded border border-white/5" />
                    </div>
                ))}
            </div>
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    <label className="text-sm font-medium text-slate-300">Hidden Test Cases (Used for Evaluation)</label>
                    <button type="button" onClick={() => {
                        const newCases = [...q.hiddenTestCases, { input: '', output: '' }];
                        updateQuestion(i, 'hiddenTestCases', newCases);
                    }} className="text-xs text-blue-400 hover:text-blue-300">+ Add Hidden</button>
                </div>
                {q.hiddenTestCases.map((tc, tcIdx) => (
                    <div key={tcIdx} className="grid grid-cols-2 gap-2 p-3 bg-slate-900/80 rounded-lg border border-white/5">
                        <textarea placeholder="Input" value={tc.input} onChange={(e) => {
                            const newCases = [...q.hiddenTestCases]; newCases[tcIdx].input = e.target.value; updateQuestion(i, 'hiddenTestCases', newCases);
                        }} className="w-full bg-slate-950 p-2 text-sm rounded border border-white/5" />
                        <textarea placeholder="Expected Output" value={tc.output} onChange={(e) => {
                            const newCases = [...q.hiddenTestCases]; newCases[tcIdx].output = e.target.value; updateQuestion(i, 'hiddenTestCases', newCases);
                        }} className="w-full bg-slate-950 p-2 text-sm rounded border border-white/5" />
                    </div>
                ))}
            </div>
            ...
            <div className="space-y-2">
                <label className="text-sm font-medium text-slate-300">Marks for this Coding Problem</label>
                <input
                    type="number"
                    value={q.marks}
                    onChange={(e) => updateQuestion(i, 'marks', e.target.value)}
        ...
        <div className="min-h-screen bg-slate-950 p-6 lg:p-12 text-slate-50">
            <div className="max-w-4xl mx-auto space-y-8">
                ...
                <div>
                    ...
                    <p className="text-slate-400 mt-2">Configure a new assessment for your students.</p>
                </div>
                ...
                {step === 1 && (
                    <form onSubmit={handleCreateExam} className="glass border border-white/5 rounded-2xl p-6 sm:p-10 space-y-6">
                        <h2 className="text-xl font-semibold mb-6">Step 1: Exam Configuration</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setExamDetails({ ...examDetails, scheduledDate: e.target.value })}
                            ...
                            <div className="space-y-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setExamDetails({ ...examDetails, scheduledTime: e.target.value })}
                                    className="w-full px-4 py-2.5 bg-slate-900/50 border border-white/10 rounded-xl focus:ring-2 focus:ring-blue-500/50 [color-scheme:dark]"
                                />
                            </div>
                        </div>
                ...
                {step === 2 && (
                    <div className="space-y-6">
                        <div className="flex items-center justify-between glass border border-white/5 rounded-xl p-4">
                            <h2 className="text-lg font-semibold">Step 2: Add {examDetails.type} Questions</h2>
                            <p className="text-sm text-slate-400">Total Questions: {questions.length}</p>
                        </div>

                        {questions.map((q, i) => (
                            <div key={i} className="glass border border-white/5 rounded-2xl p-6 sm:p-8 relative">
                                <div className="absolute top-6 right-6 flex items-center gap-2">
                                    <span className="text-xs font-bold text-slate-500 tracking-wider">Q{i + 1}</span>
                                    {questions.length > 1 && (
                                        <button onClick={() => removeQuestion(i)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors">
                                            <Trash2 size={16} />
                                        </button>
                                    )}
                                </div>
